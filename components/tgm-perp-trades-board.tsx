...
export function TGMPerpTradesBoard() {
  ...
  const [selectedOrderTypes, setSelectedOrderTypes] = useState<Record<"MARKET" | "LIMIT", boolean>>({ MARKET: true, LIMIT: true });
  const [valueMin, setValueMin] = useState<string>("");
  const [valueMax, setValueMax] = useState<string>("");

  const sortFields: Array<{ value: TGMPerpTradesSortField; label: string }> = [
    ...
    try {
      const filters: TGMPerpTradesFilters = {
        ...
        action: Object.entries(selectedActions)
          .filter(([_, selected]) => selected)
          .map(([action]) => action as "Add" | "Reduce" | "Open" | "Close"),
        order_type: Object.entries(selectedOrderTypes)
          .filter(([_, selected]) => selected)
          .map(([type]) => type as "MARKET" | "LIMIT"),
      ...
      const resp: TGMPerpTradesResponse = await fetchTGMPerpTrades({
      ...
      setError(e?.message || "Failed to load perp trades data");
    } finally {
      setLoading(false);
    }
  }
  ...
  }, [page, perPage, sortBy, sortDirection, from, to, selectedSides, selectedActions, selectedOrderTypes, valueMin, valueMax]);
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-white font-normal">Token Mode Perp Trades</span>
        ...
        <div className="px-4 py-4 border-b border-[#20222f] bg-[#1a1c29]">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-1.5">
                {(["Add", "Reduce", "Open", "Close"] as const).map((action) => (
                  <Button
                    key={action}
            ...
            {/* Order Type Filter */}
            <div className="space-y-2">
              <label className="text-[10px] text-gray-400 uppercase tracking-wide">Order Type</label>
              <div className="flex flex-wrap gap-1.5">
                {(["MARKET", "LIMIT"] as const).map((type) => (
                  <Button
                    key={type}
                    variant={selectedOrderTypes[type] ? "secondary" : "outline"}
                    size="sm"
                    className={`h-7 text-xs ${selectedOrderTypes[type] ? "bg-yellow-500/20 border-yellow-500/50 text-yellow-300" : "border-[#20222f] text-gray-400 hover:bg-[#20222f]"}`}
                    onClick={() => setSelectedOrderTypes((prev) => ({ ...prev, [type]: !prev[type] }))}
                  >
                    {type}
                  </Button>
                ))}
            ...
            <div className="mb-6">
              <div className="flex items-center gap-2 mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-white">Perp Trades</span>
          ...
          {!loading && data.length === 0 && !error && (
            <div className="flex items-center justify-center py-12">
              <div className="text-center">
                ...
                <div className="text-sm text-gray-400">Enter a token symbol to view perp trades</div>
              </div>
            </div>
          )}
