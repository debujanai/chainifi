...
export function SocialIcon({ platform, className = "", size = 10 }: { platform: string; className?: string; size?: number }) {
    ...
    if (p === 'discord') return <DiscordIcon className={className} size={size} />;
    if (p === 'telegram') return <TelegramIcon className={className} size={size} />;
    if (p === 'tiktok') return <TikTokIcon className={className} size={size} />;

    // Fallback to null for unknown platforms
    return null;
}
